...
static NTSTATUS dequeue_device_op( void *args )
{
    ...
    *dst = src->device_info;
    ...
    if (dst->label) dst->label = (char *)dst + (src->device_info.label - (char *)&src->device_info);
...
static NTSTATUS wow64_dequeue_device_op(void *args)
{
    ...
    if (src.serial)
        ...
        dst->serial = 0;
    if (src.label)
        dst->label = PtrToUlong(dst->str_buffer + (src.label - src.str_buffer));
    else
        dst->label = 0;
    if (src.guid)
