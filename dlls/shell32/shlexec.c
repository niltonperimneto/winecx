...
static UINT_PTR SHELL_CrossOverFallback(const WCHAR* lpFile, const WCHAR* lpVerb, WCHAR* cmdFormat, int cmdFormatSize, UINT_PTR retval)
{
...
HINSTANCE WINAPI FindExecutableW(LPCWSTR lpFile, LPCWSTR lpDirectory, LPWSTR lpResult)
{
    ...
    if (retval <= 32)
    {
        retval=SHELL_CrossOverFallback(lpFile, L"open", lpResult, MAX_PATH*sizeof(*lpResult), retval);
        if (retval > 32)
        {
            /* Remove double quotation marks and command line arguments */
            if (*lpResult == '"')
...
static BOOL SHELL_execute( LPSHELLEXECUTEINFOW sei, SHELL_ExecuteW32 execfunc )
{
	...
	*wszDir = '\0';
    ...
    else
    {
        ...
        retval=SHELL_CrossOverFallback(lpFile, sei_tmp.lpVerb, wCmdFormat, sizeof(wCmdFormat), retval);
        if (retval > 32)
        {
